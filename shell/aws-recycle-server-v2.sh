...
unset INSTANCE_TO_CYCLE INSTANCE_RESOURCE_NAME ASSUME_ENVIRON SCRIPTPATH SALTUSER
while [[ $# -gt 0 ]]
do
    key="$1"
...
if [[ $RECYCLE_INSTANCE_NAME != null ]]; 
  then 
    #Found an instance, rock on.
      printf "\n%s\n" "$BANNER_STRING"
    echo "module.$INSTANCE_RESOURCE_NAME.aws_instance.windows_server[$INSTANCE_TO_CYCLE]"
    ...
    if [[ $EXECUTE_DEREGISTRATION == "true" ]]
      then 
        ...
        do
          echo "Still have bindings, looping and sleeping 10"
          sleep 10
          CURRENT_BINDINGS=$(source "$SCRIPTPATH"/checkterraformtargetgroupmembers.sh --assume-role "$ASSUME_ENVIRON" --assume-role-silently "$ASSUME_SILENT" --script-path "$SCRIPTPATH" | \
            grep -E "$RECYCLE_INSTANCE_ID")
          echo "Current Registrations: $CURRENT_BINDINGS"
        ...
        if [[ $EXECUTE_TERMINATION == "true" ]]
          then 
            printf "\n%s\nTainting Resource\n%s\n" "$BANNER_STRING" "$BANNER_STRING"
            terraform taint "module.$INSTANCE_RESOURCE_NAME.aws_instance.windows_server[$INSTANCE_TO_CYCLE]"

            printf "\n%s\nApply Changes\n%s\n" "$BANNER_STRING" "$BANNER_STRING"
            # terraform apply -auto-approve
            terraform apply -target="module.$INSTANCE_RESOURCE_NAME.aws_instance.windows_server[$INSTANCE_TO_CYCLE]" #-auto-approve
          else
    ...
    echo "no entry found for module.$INSTANCE_RESOURCE_NAME.aws_instance.windows_server[$INSTANCE_TO_CYCLE]"
fi
